...
def cli_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--all-signal", action="store_true", help="Send all possible signals"
    )
    parser.add_argument(
        "--thread",
        type=thread_number,
...
def thread_number(nbt: int) -> int:
    ...
    if not isinstance(nbt, int):
        raise argparse.ArgumentTypeError(
            "Given number of thread ({}) is an {} not an integer.".format(
                nbt, type(nbt)
            )
        )
    if int(nbt) < 1:
...
def elem_in_list(elements: iter):
    def valid_element(element: str) -> str:
        ...
        if element not in elements:
            raise argparse.ArgumentTypeError(
                f"Value {element} is unexpected. Valid inputs: "
